...
import { Layout, Form, Button, Input, Row, Col, Select, DatePicker, TreeSelect, Table, Tooltip, Pagination, ConfigProvider } from "antd";
import "./module.less";

import { queryAllType, querySystemOffice, querySendMy, queryMoneyData, queryUserList } from "../api/asset";
...
const SendMy = (props) => {
  const { cswdFolw } = props?.customConfig || {};
  ...
  const getTabelData = (queryForm) => {
    ...
    // 表格数据
    querySendMy(dataForm).then(async (res) => {
  ...
  const getTableMoneyData = async (row) => {
    ...
    if (cswdFolw) {
      _cswdFolw = JSON.parse();
    }

    let dataForm = {
      flowInstIds: [row.flow_inst_id],
      flowFormColumns: _cswdFolw,
    };
