...
const App = props => {
    ...
    const getShowInfo = async () => {
        ...
        // setPhone(getUserData?.data?.mobile);
    ...
    const bindOnSubmit = async (params, { history }, afterGetToken) => {
        getAfterGetToken = afterGetToken;
        ...
        return new Promise(resolve => {
            ...
            loginAccount4Application(loginForm)
                .then(res => {
                    getUser().then(resp => {
                        ...
                        console.log(99995678, res);
                        setAfterLoginData(login);
                        // 判断是否是有留资页面
                        if (res.data && res.data?.endNodeResult?.needMessage && phone) {
                            getShowInfo();
                        } else {
                            if (inviteUrlStatus) {
                                loginAfter();
    ...
    const loginAfter = () => {
        getUser().then(res => {
            ...
            if (loginName.length > 7) {
                name =
                    loginName.slice(0, 3) + '****' + loginName.slice(7, loginName.length);
            } else if (loginName.length < 8 && loginName.length > 3) {
                let dic = {
                    4: '*',
                    5: '**',
                    6: '***',
                ...
                name = loginName.slice(0, 3) + dic[loginName.length];
            }
            setUserName(name);
            setShow(true);
        });
